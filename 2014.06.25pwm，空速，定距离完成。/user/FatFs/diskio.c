...
uint32_t Mass_Blck_Size,Mass_Blck_Count;
...
)
{
	...
	{
		Mass_Blck_Count = (mSDCardInfo.SD_csd.DeviceSize + 1) * 1024;
	}
	else	/* 普通SD卡, 最大4G */
	{
		NumberOfBlocks  = ((1 << (mSDCardInfo.SD_csd.RdBlockLen)) / 512);
		Mass_Blck_Count = ((mSDCardInfo.SD_csd.DeviceSize + 1) * (1 << DeviceSizeMul) << (NumberOfBlocks/2));
	}
