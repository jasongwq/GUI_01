...
/*			  输出占空比计算公式：TIM_Pulse/(TIM_Period	+ 1) * 100%
*			  
*/
...
{  
    ...
    if(OC1==1)
    {
        ...
        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;			  //IO端口的第6位
	...
	if(OC2==1)
    {
        ...
        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;			  //IO端口的第7位
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	      //端口模式为复用推拉输出方式
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	  //翻转速度为50M
        GPIO_Init(GPIOA, &GPIO_InitStructure);
    }
	if(OC3==1)
    {
        ...
        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;			  //IO端口的第8位
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	      //端口模式为复用推拉输出方式
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	  //翻转速度为50M
        GPIO_Init(GPIOA, &GPIO_InitStructure);
    }
	if(OC4==1)
    {
        ...
        GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;			  //IO端口的第8位
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	      //端口模式为复用推拉输出方式
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	  //翻转速度为50M
        GPIO_Init(GPIOA, &GPIO_InitStructure);
    }
...
{  
    ...
    TIM2_OCInitStructure.TIM_Pulse = 5000; 							 //设置脉宽
...
{  
    ...
    TIM3_OCInitStructure.TIM_Pulse = 5000; 							 //设置脉宽
...
{  
    ...
    TIM4_OCInitStructure.TIM_Pulse = 5000; 							 //设置脉宽
...
{  
    ...
    TIM5_OCInitStructure.TIM_Pulse = 5000; 							 //设置脉宽
...
{  
    ...
    TIM8_OCInitStructure.TIM_Pulse = 5000; 							 //设置脉宽
